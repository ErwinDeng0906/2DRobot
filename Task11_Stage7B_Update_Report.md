...
flowchart TD
    ...
    B --> C["计算中位数图像误差 e"]
    C --> D{"视觉估计剩余XY距离是否 <= 1.0 mm"}
    D -- 是 --> E["完成：显示已抵达1mm以内，不再发送XY运动"]
    D -- 否 --> F{"当前位置是否进入Task9精细域"}
    F -- 否 --> G["Task11宽域模型：规划粗修正 <= 0.74 mm；执行硬限0.75 mm"]
    F -- 是 --> H["Task9局部模型：规划精修正 <= 0.24 mm；执行硬限0.25 mm"]
...
|---|---:|---:|---:|
| Task11 宽域模型 | 每轴 ±10 mm | 每轴 ±9.5 mm | 规划0.74 mm；执行硬限0.75 mm |
| Task9 精细模型 | 每轴 ±2 mm | 每轴 ±1.8 mm | 规划0.24 mm；执行硬限0.25 mm |
...
- 累计 XY 路径不超过 50 mm；
- 宽域规划单步不超过0.74 mm，ActionWorker执行/到位后反算硬上限仍为0.75 mm；
- 精细域规划单步不超过0.24 mm，ActionWorker执行/到位后反算硬上限仍为0.25 mm；
- 每轮都重新读取控制器状态；
- 每轮固定 J3 和标定时的绝对 Rz；
- 不包含 Z、DO、真空或接触动作。

Stage7B只要求测量起点和Cartesian终点位于相应Jacobian有效域，不再使用
`sequential_intermediate_inside_local_domain` 拒绝控制器自然的逐轴J1/J2中间姿态。
逐轴瞬态XY长度、绝对Rz、终点域、可达性和FK闭合仍会独立复核；其它门失败时，
程序仍可依次尝试原命令的 `1、0.5、0.25、0.125` 倍，全部失败则停止。
...
| `src/scara/ui/action_worker.py` | 新增 Stage7B 专用运行请求协议；在每次运动前重新读取控制器状态，独立复核hash、模型层级、终点、J3/Rz、端点域和逐轴瞬态；Stage7B允许自然的J1/J2中间姿态越出模型域，Stage7A仍保持原门；支持收敛后正常提前结束。 |
...
Task11 已在真实硬件上完成，随后按第10节记录的已授权重复性阈值重新处理；
`src/scara/calib/camera1_wide_xy_jacobian.json` 已成功安装。Stage7B仍会在正式宽域
模型不存在、质量门失败或任一锁定hash不匹配时拒绝ARM。
...
## 10. 2026-08-15 实测数据重处理记录

`Trajectory Photos/260815100348` 的Task11采集完成了66次访问和330张照片。
原始宽域拟合仅因 `node_repeatability=1.0410506 px` 高于当时的
`0.75 px` 限制而拒绝安装；训练RMS、独立验证RMS、独立验证最大误差、
Jacobian条件数、最小奇异值、验证Jacobian一致性、预测下降及可逆性均通过。

经操作者授权，`maximum_node_repeatability_px` 已调整为 `1.50 px`；
其余质量门保持不变。使用
`tools/reprocess_task11_wide_jacobian.py` 对该运行目录的既有 `points.json`
进行离线重新拟合：未移动机械臂、未拍摄新图，并先核验内参、托盘、吸盘标定
和Task9 Jacobian的锁定哈希。重处理结果为 `success`，正式文件已安装到
`src/scara/calib/camera1_wide_xy_jacobian.json`。运行目录保留了原始失败结果的
`camera1_wide_xy_jacobian_before_repeatability_policy_override.json` 备份，以及
新结果的 `reprocessing` 审计字段。

## 11. 2026-08-15 Stage7B 试运行门限策略调整

为先完成宽域模型的可运行性验证，再逐步收紧精度策略，操作者授权了以下
Stage7B专用门限；Stage7A参数不随之改变：

| Stage7B门 | 新限制 |
|---|---:|
| `session_path_limit` | 累计XY路径≤50 mm |
| `error_window_stable` | RMS≤1.5 px且最大偏差≤3.0 px |
| `robot_stationary_window` | XY扩散≤0.20 mm；每关节峰峰扩散≤0.20 |
| `robot_state_freshness` | 图像/状态配对年龄≤1.00 s |
| `request_state_matches_measurement` | XY差≤0.20 mm；最大关节差≤0.20 |
| `calibration_anchors_match` | Task9/Task11 P22锚点距离≤0.50 mm |
| `previous_response_innovation` | 创新残差范数≤2.00 px |
| `previous_command_tracking` | XY命令跟踪误差≤0.15 mm |

Stage7B不再生成或阻塞于
`sequential_intermediate_inside_local_domain`。这只允许控制器自然的逐轴J1/J2
中间姿态暂时离开Jacobian模型域；测量起点和命令终点仍必须在域内，逐轴瞬态
XY长度、绝对Rz、单步、累计路径、IK/FK、J3、报警、急停和控制器状态等门继续
生效。ActionWorker根据固定的Stage7B `request_key`应用此策略，导入的任务文件
不能把Stage7A的对应门关闭。

## 12. 2026-08-15 宽域规划与执行上限分离

实测 `260815115827` 的第二次宽域命令为精确的0.75 mm；控制器到位关节角经过
显示/协议量化后，FK反算实际步长为0.7501569 mm。机械臂到位误差仅约
0.000054°，但旧逻辑仍因反算结果比0.75 mm高0.0001569 mm而停止。

现将Task11宽域候选的规划向量上限设为0.74 mm，同时保留ActionWorker的
0.75 mm执行硬上限。0.01 mm只作为控制器量化和FK舍入余量，不放宽物理运动
边界；Task9精细层0.25 mm限制、终点域、逐轴瞬态、J3/Rz、IK/FK、报警和急停
等门均保持不变。

## 13. 2026-08-15 单点有限闭环终止条件与精细余量

实测 `260815121104` 连续完成7次0.74 mm宽域运动，第8轮成功切换到Task9
精细模型。该轮计划0.25 mm，控制器回读关节经FK反算为0.2500077 mm，仅高出
约0.0000077 mm，因此旧逻辑再次产生到位后数值假失败；这不是正常到达提示。

本次新增：

- UI按钮及运行状态统一使用“单点有限闭环”；
- 使用当前模型的完整抵消量 `||-J^{-1}e||` 作为吸盘到目标的视觉估计剩余距离；
- 剩余距离≤1.0 mm时不再运动，正常返回“已经抵达距离目标点1mm以内”；
- Task9精细规划上限改为0.24 mm，ActionWorker执行硬上限仍为0.25 mm；
- Stage7B走共享ActionWorker时，所有可见响应使用“单点有限闭环”，不再错误显示
  “Stage7A实际到位状态未通过运动安全复核”。

0.24/0.25 mm的0.01 mm余量只处理控制器关节量化和FK舍入；J3/Rz、终点域、
逐轴瞬态、IK/FK、报警、急停和控制器状态门均未放宽。
